        ...
        static FormatterLocator()
        {
            foreach (var ass in AppDomain.CurrentDomain.GetAssemblies())
            {
                ...
                catch (TypeLoadException)
                {
                    ...
                    {
                        Console.WriteLine("A TypeLoadException occurred when FormatterLocator tried to load types from assembly '" + ass.FullName + "'. No serialization formatters in this assembly will be found. Serialization will be utterly broken.");
                    }
                }
                catch (ReflectionTypeLoadException)
                {
                    if (ass.GetName().Name == "OdinSerializer")
                    {
                        Console.WriteLine("A ReflectionTypeLoadException occurred when FormatterLocator tried to load types from assembly '" + ass.FullName + "'. No serialization formatters in this assembly will be found. Serialization will be utterly broken.");
                    }
                }
...
#pragma warning restore CS0618 // Type or member is obsolete
            ...
            if (EmitUtilities.CanEmit)
            {
                Console.WriteLine("Fallback to reflection for type " + type.Name + " when emit is possible on this platform.");
            }
