...
namespace OdinSerializer
{
    ...
    public static class FormatterEmitter
    {
        ...
        {
#if !CAN_EMIT
        Console.WriteLine("Cannot use Reflection.Emit on the current platform. The FormatterEmitter class is currently disabled. Check whether emitting is currently possible with EmitUtilities.CanEmit.");
        return null;
#else
            if (type == null)
            {
            ...
            if (Formatters.TryGetInnerValue(policy, type, out result) == false)
            {
                lock (LOCK)
                {
                    if (Formatters.TryGetInnerValue(policy, type, out result) == false)
                    {
                        ...
                        {
                            Console.WriteLine("The following error occurred while emitting a formatter for the type " + type.Name);
