    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 frag(v2f i) : SV_Target
            {
                ...
                half4 fill = tex2D(_MainTex, uv);

                col *= _Color;
                fill *= _FillColor;

                half mainBlit = max(0, sign(uv.y - _Fill));
                half fillBlit = max(0, sign(_Fill - uv.y));

                half4 res = half4(0, 0, 0, 0);
                res += col * half4(mainBlit, mainBlit, mainBlit, mainBlit);
                res += fill * half4(fillBlit, fillBlit, fillBlit, fillBlit);
