...
namespace NodeEditorFramework
{
    ...
    public static partial class NodeEditorSaveManager
    {
...
#if UNITY_5_3_OR_NEWER || UNITY_5_3
                    .Select((NodeCanvasSceneSave save) =>
                            SceneManager.sceneCount > 1? (save.gameObject.scene.name + "::" + save.saveName) : save.saveName)
#else
        ...
        public static void SaveSceneNodeCanvas(string saveName, ref NodeCanvas nodeCanvas, bool createWorkingCopy, bool safeOverwrite = true)
        {
            ...
            if (nodeCanvas == null)
                throw new System.ArgumentNullException("Cannot save NodeCanvas: The specified NodeCanvas that should be saved as '" + saveName +
                    "' is null!");
        ...
        public static void SaveNodeCanvas(string path, ref NodeCanvas nodeCanvas, bool createWorkingCopy, bool safeOverwrite = true)
        {
            ...
            if (nodeCanvas.allowSceneSaveOnly)
                throw new System.InvalidOperationException(
                    "Cannot save NodeCanvas: NodeCanvas is marked to contain scene data and cannot be saved as an asset!");

            nodeCanvas.Validate();

            if (nodeCanvas.livesInScene)
            {
                Debug.LogWarning("Attempting to save scene canvas '" + nodeCanvas.name + "' to an asset, references to scene object may be broken!" +
                    (!createWorkingCopy? " Forcing creation of working copy!" : ""));
        ...
        public static NodeCanvas CreateWorkingCopy(NodeCanvas nodeCanvas, bool editorStates = true)
        {
            ...
            foreach (NodeEditorState state in nodeCanvas.editorStates)
            {
                if (state.selectedNodes.Count > 0)
                    state.selectedNodes[0] = ReplaceSO(allSOs, clonedSOs, state.selectedNodes[0]);
            }
        ...
        /// </summary>
        private static void AddClonedSOs<T>(List<ScriptableObject> scriptableObjects, List<ScriptableObject> clonedScriptableObjects,
        IEnumerable<T> initialSOs) where T : ScriptableObject
        {
        ...
        /// </summary>
        private static T AddClonedSO<T>(List<ScriptableObject> scriptableObjects, List<ScriptableObject> clonedScriptableObjects, T initialSO)
        ...
        /// </summary>
        private static T ReplaceSO<T>(List<ScriptableObject> scriptableObjects, List<ScriptableObject> clonedScriptableObjects, T initialSO)
                where T : ScriptableObject
        {
            if (initialSO == null)
                return null;
            ...
            else
            {
                ...
                if (!SO)
                    ...
                    Debug.LogError("GetWorkingCopy: ScriptableObject " + initialSO.name + " is not of the same type as copy " +
                        clonedScriptableObjects[soInd].name + "!");
            }
