...
namespace NodeEditorFramework
{
    ...
    {
        // Properties
        public override ConnectionShape shape
        {
            get
            {
                return ConnectionShape.Bezier;
            }
        }

        // Connections
        new public List<ConnectionKnob> connections
        {
            get
            {
                return _connections.OfType<ConnectionKnob>().ToList();
            }
        }

        // Knob Style
        protected override Type styleBaseClass
        {
            get
            {
                return typeof (ConnectionKnobStyle);
            }
        }

        new protected ConnectionKnobStyle ConnectionStyle
        {
            get
            {
        ...
        private float knobAspect
        {
            get
            {
                return knobTexture != null? ((float) knobTexture.width) / knobTexture.height : 1;
            }
        }
        ...
        // Knob Position
        private NodeSide defaultSide
        {
            get
            {
                return direction == Direction.Out? NodeSide.Right : NodeSide.Left;
            }
        }
        ...
        protected virtual void ReloadTexture()
        {
            ...
            knobTexture = ConnectionStyle == null? Texture2D.whiteTexture
                    : (direction == Direction.Out? ConnectionStyle.OutKnobTex : ConnectionStyle.InKnobTex);
        }
    ...
    public class ConnectionKnobAttribute: ConnectionPortAttribute
    {
        ...
        public ConnectionKnobAttribute(string name, Direction direction, ConnectionCount maxCount, NodeSide nodeSide, float nodeSidePos = 0):
                base(name, direction)
        {
            Setup(maxCount, nodeSide, nodeSidePos);
        }

        public ConnectionKnobAttribute(string name, Direction direction, string styleID, NodeSide nodeSide, float nodeSidePos = 0): base(name,
            direction, styleID)
        {
            Setup(nodeSide, nodeSidePos);
        }

        public ConnectionKnobAttribute(string name, Direction direction, string styleID, ConnectionCount maxCount, NodeSide nodeSide,
        float nodeSidePos = 0): base(name, direction, styleID)
        {
            Setup(maxCount, nodeSide, nodeSidePos);
        }
    ...
    {
        public const string InKnobTexPath = "Textures/In_Knob.png";
        public const string OutKnobTexPath = "Textures/Out_Knob.png";
        ...
        protected void LoadKnobTextures()
        {
            ...
            if (InKnobTex == null || OutKnobTex == null)
                Debug.LogError("Invalid style '" + Identifier + "': Could not load knob textures from '" + InKnobTexPath + "' and '" +
                    OutKnobTexPath + "'!");
        }
